...
public:
    ...
    static std::shared_ptr<Person> getPerson(const std::string firstName, const std::string lastName, const int id,
         const int age,const type t){
         if(t==PERSON){
            return std::make_shared<Person>(firstName,lastName,id,age);
         }
         //return nullptr;
    }
